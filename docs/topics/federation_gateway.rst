...
Implementation
^^^^^^^^^^^^^^
Our `quick start UI <https://www.nuget.org/packages/VkSoft.Community.IdentityServer10.Quickstart.UI>`_ utilizes some of the below features. Also check out the :ref:`external authentication quickstart <refExternalAuthenticationQuickstart>` and the 
